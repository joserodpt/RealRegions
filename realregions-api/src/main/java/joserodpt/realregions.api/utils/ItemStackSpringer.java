...
public class ItemStackSpringer {
    ...
    public static Enchantment getEnchantmentByName(String name) {
        switch (name.toLowerCase()) {
            ...
            default:
                RealRegionsAPI.getInstance().getLogger().severe(name + " isn't a known Enchantment (is this a bug?) Skipping this enchant.");
                return null;
        }
    }
    ...
    public static List<Map<String, Object>> getItemsSerialized(ItemStack[] items) {
        return IntStream.range(0, items.length)
                .filter(i -> items[i] != null)
                .mapToObj(i -> getItemSerialized(i, items[i]))
                .collect(Collectors.toList());
    }
}